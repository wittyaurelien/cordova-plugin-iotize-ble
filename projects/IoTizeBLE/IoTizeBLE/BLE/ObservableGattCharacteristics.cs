...
namespace IoTizeBLE
{
    ...
    {
        public ReceivedEventArgs(byte[] response) { Response = response; }
        public byte[] Response { get; private set; } // readonly
    };

    /// <summary>
    /// Wrapper around <see cref="GattCharacteristic"/>  to make it easier to use
    /// </summary>
    internal class ObservableGattCharacteristics : INotifyPropertyChanged
    {
        /// <summary>
        ...
        // the delegate (if using non-generic pattern).
        public delegate void ReceivedEventHandler(object sender, ReceivedEventArgs e);


        // Declare the event.
        public event ReceivedEventHandler ResponseEvent;
        ...
        private void SetValue(IBuffer buffer)
    {
        ...
        ResponseEvent?.Invoke(this, new ReceivedEventArgs(data));
    ...
    private void OnPropertyChanged(PropertyChangedEventArgs e)
    {
        if (e.PropertyName == "DisplayType")
        {
            Log.WriteLine($"{this.Name} - DisplayType set: {this.DisplayType.ToString()}");
        }
